...
toMessage (Waken m) = Just $ "You hear shuffling nearby."
...
toMessage (Equipped m item) | isPlayer m = Just $ "You have equipped up the " ++ show item ++ "."
toMessage (EquipmentRemoved m item) | isPlayer m = Just $ "You have removed the " ++ show item ++ "."
